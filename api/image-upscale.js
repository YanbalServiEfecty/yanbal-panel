...
// v17: scale forzado a 2 (scale=4 destruye el texto de las etiquetas); face_enhance=false.
// El cliente debe decidir si siquiera llama este endpoint (ver v17UpscaleConGuard).
export const config = {
  maxDuration: 60,
...
export default async function handler(req, res) {
  ...
  try {
    const { imageB64, mediaType = 'image/jpeg', scale } = req.body || {};
    if (!imageB64) return res.status(400).json({ error: 'Falta imageB64' });

    // v17: scale forzado a 2 siempre. scale=4 en Real-ESRGAN destruye el texto de las
    // etiquetas de los productos (borroso, artefactos). scale=2 mantiene legibilidad.
    // Si el cliente pide scale=4 explícitamente, capeamos a 2 y devolvemos un warning.
    const scaleSolicitado = parseInt(scale) || 2;
    const scaleFinal = 2;
    const scaleCapeado = scaleSolicitado > 2;

    const dataUrl = `data:${mediaType};base64,${imageB64}`;

    // Modelo: Real-ESRGAN x2 por nightmareai. Version fija para estabilidad.
    ...
    if (createData.status === 'succeeded' && createData.output) {
      return res.status(200).json({
        ...
        status: 'succeeded',
        scaleUsado: scaleFinal,
        scaleCapeado: scaleCapeado ? `Solicitado ${scaleSolicitado}, limitado a ${scaleFinal} para preservar legibilidad de texto` : undefined,
      });
    }
